...
1. **Known plaintext attack:** If we know `P1` and have `C1`, we can recover **`K = (C1 - P1) mod 256`**
2. **Decrypt unknown message:** With the recovered keystream, we can decrypt **`P2 = (C2 - K) mod 256`**

### **Attack Steps**

1. **Identify messages:** The longer message (300 bytes) contains **"LOL" repeated**
2. **Create crib:** Generate the known plaintext **`"LOL" * 100`**
3. **Recover keystream:** Calculate **`K = (C_lol - crib) mod 256`**
4. **Decrypt flag:** Calculate **`flag = (C_flag - K[:69]) mod 256`**
...
def solve_with_known_data():
    """**Solve with known challenge data**"""
    # Data from server
...
```python
# **Step-by-step keystream recovery**
def demonstrate_keystream_recovery():
    # Sample data (first few bytes)
...
```bash
python3 solve.py --flag-hex "86b5666d964f38b7..." --lol-hex "8fb06c3e81691fd4..."
```

### **Custom Server**

```bash
python3 solve.py --host ctf.ac.upt.ro --port 9280
```
